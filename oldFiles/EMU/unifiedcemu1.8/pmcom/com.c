...
*/
static void QUEUEPut(struct TQueue *pQueue, char *pBuf, int nSize)
{
  int nToCopy;
  ...
  memcpy(pQueue->pBuf + pQueue->nTail, pBuf, nToCopy);

  if ((nSize -= nToCopy) > 0)
  {
    /* Rest of the bytes at the start of the buffer */
    memcpy(pQueue->pBuf, pBuf + nToCopy, nSize);
    pQueue->nTail = nSize;
  }
  else
  {
...
static void QUEUEGet(struct TQueue *pQueue, char *pBuf, int nSize)
{
  ...
  memcpy(pBuf, pQueue->pBuf + pQueue->nHead, nToCopy);

  if ((nSize -= nToCopy) > 0)
  {
    memcpy(pBuf + nToCopy, pQueue->pBuf, nSize);
    pQueue->nHead = nSize;
  }
  else
  {
...
static void COMHandler(struct COMDesc *pCOM)
{
  ...
  while (((nIntID = inportb(IIR(pCOM))) & 1) == 0)
	...
	++pCOM->nStat;
	#endif
	cStat.nStat = inportb(LSR(pCOM)) & 0x1e;
	...
	{
	  QUEUEPut(&pCOM->RXQueue, (char *)&c, 1);
	  ASSERT(QUEUECalcFree(&pCOM->RXStatQueue) >= sizeof(struct COMStat));
	  QUEUEPut(&pCOM->RXStatQueue, (char *)&cStat, sizeof(struct COMStat));
	}
	...
	*/
	...
	if (nToPut > 0)
	{
	  ...
	  do
	  {
	    QUEUEGet(&pCOM->TXQueue, (char *)&c, 1);
	    outportb(THR(pCOM), c);
	    QUEUEGet(&pCOM->TXStatQueue, (char *)&cStat, sizeof(struct COMStat));
	    nLastBit9 = cStat.nBit9;
	  }
	  while (--nToPut);
  ...
  int nStopBits, int nFlowControl, void (*EventHandler)(int nEvent))
{
  ...
  */
  ...
  pRXStatQueue = NULL;
  pTXStatQueue = NULL;
  pRXQueue = malloc(nRXQueueSize + 1);  /* + 1: As Head never should tread on Tail */
  pTXQueue = malloc(nTXQueueSize + 1);
  pRXStatQueue = malloc(nRXQueueSize * sizeof(struct COMStat) + 1);
  pTXStatQueue = malloc(nTXQueueSize * sizeof(struct COMStat) + 1);
...
*/
void static COMActivateTransmition(int nCOM)
{
  struct COMDesc *pCOM;
  ...
  int nSize, int *nCount)
{
  ...
  {
    ENTER_QUEUE_CRITICAL_SECTION(pCOM);
    QUEUEPut(&pCOM->TXQueue, (char *)pBuf, nStoreCount);
    if (pStatBuf != NULL)
...
int COMTXBufferFree(int nCOM)
{
  ...
  ASSERT(nFree == nStatFree / sizeof(struct COMStat));
...
int COMTXBufferUsed(int nCOM)
{
  ...
  ASSERT(nUsed == nStatUsed / sizeof(struct COMStat));
